...
CharWin * win;
  ...
  print(win,inputstring,10);
  print(win, " ", 10);
  val = atoi(tmpstring);
  for (i=0; i<=val; i++){
     itoa(i,charstring,10);
     print(win, charstring, 10);
     print(win, " ", 10);
   }
  print(win, "\n", 10);
}
...
cwin_clear(&msgwin);
cwin_putat_string(&msgwin,0,0,"TMPSTRING IS ",10);
...
itoa(foundLink->key ,charstring,10);
len = strlen(charstring);
charstring[len] = 32; charstring[++len]=0;
cwin_putat_chars(&outputwin, oxpos, 1,  charstring , len, 5);
oxpos = oxpos + len +1 ;
itoa(foundLink->data,charstring,10);
len = strlen(charstring);
charstring[len] = 32; charstring[++len]=0;
cwin_putat_chars(&outputwin, oxpos, 1,  charstring , len, 5);
oxpos = oxpos + len +1 ;
